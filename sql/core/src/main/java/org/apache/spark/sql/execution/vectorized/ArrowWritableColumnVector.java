...
public final class ArrowWritableColumnVector extends WritableColumnVector {
  ...
  public static ArrowWritableColumnVector[] allocateColumns(int capacity, StructType schema) {
    ...
    for (int i = 0; i < fieldVectors.size(); i++) {
      vectors[i] = new ArrowWritableColumnVector(new_root, fieldVectors.get(i), i, capacity);
    }
    return vectors;
  }

  public ArrowWritableColumnVector(VectorSchemaRoot root, ValueVector vector, int ordinal, int capacity){
  ...
  public ArrowWritableColumnVector(int capacity, DataType dataType) {
    ...
    ValueVector vector = fieldVectors.get(0);
    vector.setInitialCapacity(capacity);
    vector.allocateNew();
  ...
  public ValueVector getValueVector() {
    return root.getFieldVectors().get(ordinal);
  }
